...
def render_header(ticker_items=None):
    """Render the fixed Burdy header with optional custom ticker items."""
    if ticker_items is None:
        ticker_items = [
            "Ticketmaster Discovery v2",
            "Supabase Live Sync",
            "PostCodes.io Geolocation",
            "24 Months Event Coverage",
            "UK Events Only",
        ]
    # Double the items so the ticker loops seamlessly
    all_items = ticker_items * 2
    ticker_html = "".join(
        f'<span class="ticker-item">{item}</span><span class="ticker-sep">◆</span>'
...
@media (max-width: 768px) {{
    .burdy-header {{ padding: 0 1rem; height: 60px; }}
...
def render_footer():
    """Render the Burdy footer into a persistent st.empty() slot."""
    st.markdown("""
<style>
.burdy-footer {
    position: fixed;
